 ...
 */
export const antiFraudScannerCron = functions.scheduler.onSchedule('every day 04:00', async (event) => {
  console.log('Starting anti-fraud scanner CRON...');

  try {
    const suspiciousAffiliates: string[] = [];
    ...
    const ipMap = new Map<string, { affiliateId: string; count: number; timestamps: Date[] }>();
    ...
    const botAffiliates = new Map<string, number>();
